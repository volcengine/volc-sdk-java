        ...
        consumer.registerMessageListener(new MessageListenerConcurrently() {
            ...
            public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext context) {
                for (MessageExt msg : msgs) {
                    try {
                        Canal.Entry entry = Canal.Entry.parseFrom(msg.getBody());
                        System.out.println(String.format("sourceType:%s\nentryType:%s\nexecuteTime:%s\nschemaName:%s" +
                                        "\ntableName:%s", entry.getHeader().getSourceType(), entry.getEntryType(),
