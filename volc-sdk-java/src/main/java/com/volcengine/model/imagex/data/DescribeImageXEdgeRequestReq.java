...
@Data
public class DescribeImageXEdgeRequestReq {
    ...
    @JSONField(name = "DataTypes")
    private String dataTypes;// 状态码过滤。传入多个用英文逗号分隔。支持2xx,3xx,4xx,5xx,req_cnt
    @JSONField(name = "GroupBy")
    private String groupBy;// 维度拆分的维度值。不传表示不拆分维度。传入多个用英文逗号分隔。支持DomainName,DataType
    ...
    @JSONField(name = "DetailedCode")
    private String detailedCode;// 是否拆分状态码。不传默认为false，表示不拆分。
}
