...
public final class CreateImageAuditTaskBody  {
     ...
     * <p>图片审核任务场景。取值如下所示：</p>
     *
     * <p>- `UrlFile`：存量图片处理，进针对已有存储内的图片请求获取审核结果。传入方式是 `ResUri`方式，即在.txt 文件（审核文件）内填写了待审核图片文件 URL，并将该 txt 文件上传至指定服务后获取并传入该文件的 StoreUri。</p>
     *
     * <p>- `Url`：URL 直传场景。传入方式为 `ImageInfos` 方式，即可直接传入待审核图片的 URL 及区分标识。</p>
     *
     * <p>- `Upload`：图片上传场景，针对上传图片到指定服务下的场景。可在 `EnableAuditRange`下指定审核的范围，例如对指定上传到某目录下的图片进行审核。</p>
     */
     ...
     *
     * <p>指定前缀审核，若你希望对某个目录进行审核，请设置路径为对应的目录名，以`/`结尾。例如`123/test/`</p>
     */
     ...
     * <p>仅当 `EnableAuditRange` 取值 `1` 时，配置生效。</p>
     *
     * <p>指定前缀不审核，若你希望对某个目录不进行审核，请设置路径为对应的目录名，以`/`结尾。例如`123/test/`</p>
     */
